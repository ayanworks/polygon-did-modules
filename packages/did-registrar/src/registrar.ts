...
export class PolygonDID {
  ...
  public async create(did: string, didDoc: DidDocument) {
    try {
      ...
      const txnHash = await this.registry.createDID(parsedDid.didAddress, stringDidDoc)
      if (!txnHash) {
  ...
  public async update(did: string, didDoc: DidDocument) {
    try {
      ...
      const txnHash = await this.registry.updateDIDDoc(parsedDid.didAddress, JSON.stringify(didDoc))
      if (!txnHash) {
  ...
  public async addResource(did: string, resourcePayload: ResourcePayload) {
    try {
      ...
      const txnHash = await this.registry.addResource(parsedDid.didAddress, resourceId, stringDidDoc)
      if (!txnHash) {
  ...
  public async updateResource(did: string, resourceId: string, resourcePayload: ResourcePayload) {
    try {
      const isValidDid = validateDid(did)
  ...
  public async getResourceByDidAndResourceId(did: string, resourceId: string) {
    try {
      ...
      const linkedResource = await this.registry.getResource(parsedDid.didAddress, resourceId)
  ...
  public async getResourcesByDid(did: string) {
    try {
      ...
      const listLinkedResource = await this.registry.getAllResources(parsedDid.didAddress)
  ...
  public async estimateTxFee(method: string, argument?: string[], from?: string): Promise<EstimatedTxDetails | null> {
    try {
      if (!method) {
        throw new Error(`Method is required for estimate transaction!`)
      }
      const provider = new JsonRpcProvider(this.rpcUrl)
      const contract = new Contract(this.contractAddress, DidRegistryContract.abi, provider)
